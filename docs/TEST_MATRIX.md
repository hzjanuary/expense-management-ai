...
| US-701 | Full-stack local runtime starts backend/frontend with migrations and optional Ollama | no | yes | no | yes | in_progress | Static/runtime-file proof passed through `docker compose config`; backend/frontend quality gates passed; container build/runtime proof blocked by Docker socket permissions |
